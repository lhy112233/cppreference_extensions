...
public:
  ...
  scope_exit(scope_exit &&other) noexcept
      : impl_(std::move(static_cast<impl_ &&>(other))) {}
...
//    *
//    * @details Value-initializes the stored resource handle and the deleter.
//    The
...
//    *
//    * @throws Any exception thrown during initialization of the stored
//    resource
//    * handle or the deleter.
//    *
//    * @details  The stored resource handle is initialized with
//                 std::forward<RR>(r) if std::is_nothrow_constructible_v<RS,
//                 RR>
//    is true, otherwise r. If initialization of the stored resource handle
//    throws an exception, calls d(r). Then, the deleter is initialized with
//    std::forward<DD>(d) if std::is_nothrow_constructible_v<D, DD> is true,
//    otherwise d. If initialization of deleter throws an exception, calls
//    d(res_). The constructed unique_resource owns the resource. The program is
//    ill-formed if any of the expressions d(r), d(res_) and del_(res_) is
//    ill-formed. The behavior is undefined if any of the expressions d(r),
//    d(res_) and del_(res_) results in undefined behavior or throws an
//    exception.
//    *
...
//    *
//    * @details The stored resource handle is initialized from the one of
//    other,
...
//    exception, other is not modified. Then, the deleter is initialized with
//    the one of other, using std::move if
//    std::is_nothrow_move_constructible_v<D> is true. If initialization of the
//    deleter throws an exception and
//               std::is_nothrow_move_constructible_v<RS> is true and other owns
//    the resource, calls the deleter of other with res_ to dispose the
//    resource, then calls other.release(). After construction, the constructed
...
//    *
//    * @throws Any exception thrown during initialization of the stored
//    resource handle or the deleter. exception description
//    *
//    * @note The mechanism of these constructors ensures no leaking of
//    resources.
//    */
...
//    * @brief Destructor.
//    *
//    * @details Disposes the resource by calling the deleter with the
//    underlying
...
//               First, calls reset() to dispose the currently owned resource,
//               if
//    any. Then assigns the stored resource handle and the deleter with other's.
//    std::move is applied to the stored resource handle or the deleter of other
//    if
//               std::is_nothrow_move_assignable_v<RS> or
//    std::is_nothrow_move_assignable_v<D> is true respectively. Assignment of
//    the stored resource handle is executed first, unless
//    std::is_nothrow_move_assignable_v<D> is false and
//               std::is_nothrow_move_assignable_v<RS> is true. Finally, sets
//               *this
...
//    *
//    * @details The destructor will not execute the deleter after the call,
//    unless
...
//    * well-formed.
//    *
//    * @details Replaces the resource by calling (1) and then assigns the
//    stored
...
//    *
//    * @return The object or function pointed by the underlying resource
//    handle.
//    *
//    * @requires std::is_pointer_v<R> &&
//    !std::is_void_v<std::remove_pointer_t<R>>
//    *
//    * @details If the resource handle is not pointing to an object or a
//    function,
...
//    * @requires std::is_pointer_v<R>
//    *
//    * @details The return value is typically used to access the pointed
//    object.
//    */

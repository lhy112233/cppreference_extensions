...
public:
  ...
  EFBox(BoxRollbackHelper<Helper> helper,
        Args &&...args) noexcept(std::is_nothrow_constructible_v<EF, Args...>)
  ...
  EFBox(BoxRollbackHelper<Helper> helper,
        EFBox<EF> &&other) noexcept(std::is_nothrow_move_constructible_v<EF>)
      : ef_(std::move(other.ef_)) {
    helper.release();
  }
...
public:
  template <typename Helper, typename... Args>
  EFBox(BoxRollbackHelper<Helper> helper, Args &&...args) noexcept(
      noexcept(std::reference_wrapper<EF>{std::forward<Args>(args)...}))
      : std::reference_wrapper<EF>(std::forward<Args>(args)...) {
        throw std::runtime_error{"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"};
    helper.release();
  }

  template <typename Helper>
  EFBox(BoxRollbackHelper<Helper> helper, EFBox<EF> &&other) noexcept
      : std::reference_wrapper<EF>(
            static_cast<std::reference_wrapper<EF> &&>(other)) {
    helper.release();
  }
}; // class EFBox<EF &>
...
// class BaseScopeImpl
/**************************************/
template <typename Fn, typename EF, typename Self>
concept BaseScopeConstructConcept =
    (!std::is_same_v<std::remove_cvref_t<Fn>, Self>) &&
    std::is_constructible_v<EF, Fn>;

template <typename EF>
concept BaseScopeMoveConstructibleConcept =
    std::is_nothrow_move_constructible_v<EF> ||
    std::is_copy_constructible_v<EF>;

template <typename EF> class BaseScopeImpl : public EFBox<EF> {

public:
  template <BaseScopeConstructConcept<EF, BaseScopeImpl<EF>> Fn>
    requires(!std::is_lvalue_reference_v<Fn>) &&
            std::is_nothrow_constructible_v<EF, Fn>
  explicit BaseScopeImpl(Fn &&fn) noexcept
      : EFBox<EF>{BoxRollbackHelper{[] {}}, std::forward<Fn>(fn)} {}

  template <BaseScopeConstructConcept<EF, BaseScopeImpl<EF>> Fn>
  explicit BaseScopeImpl(Fn &&fn) noexcept(
      std::is_nothrow_constructible_v<EF, Fn &>)
      : EFBox<EF>{BoxRollbackHelper{[&fn] { std::invoke(fn); }}, fn} {}

  BaseScopeImpl(BaseScopeImpl &&other) noexcept
    requires BaseScopeMoveConstructibleConcept<EF> &&
             std::is_nothrow_move_constructible_v<EF>
      : EFBox<EF>(BoxRollbackHelper{[] {}}, std::forward<EFBox<EF>>(other)) {}

  BaseScopeImpl(BaseScopeImpl &&other) noexcept(
      std::is_nothrow_copy_constructible_v<EFBox<EF>>)
    requires BaseScopeMoveConstructibleConcept<EF>
      : EFBox<EF>{BoxRollbackHelper{[] {}}, static_cast<EFBox<EF> &>(other)} {}
...
public:
  ...
  scope_exit_impl(scope_exit_impl &&other) noexcept(noexcept(base_{
      std::move(static_cast<base_ &&>(other))}))
      : base_(std::move(static_cast<base_ &&>(other))),
  ...
  ~scope_exit_impl() noexcept {
    if (!released_) {
      std::invoke(static_cast<EF &>(*this));
    }
  }
...
//               else
//                 std::invoke(static_cast<D &>(other), static_cast<RS
//                 &>(*this));
//             }),
//         released_(other.released_) {
//     other.release();
